...
export default defineSchema({
  ...
  conversations: defineTable({
    client_user_id: v.id("users"),
    trainer_user_id: v.id("users"),
    status: v.union(
      v.literal("OPEN"),
      v.literal("BLOCKED"),
      v.literal("CONTRACTED")
    ),
    contract_valid_until: v.optional(v.number()),
    last_message_at: v.number(),
    last_message_text: v.optional(v.string()),
    client_last_read_at: v.optional(v.number()),
    trainer_last_read_at: v.optional(v.number()),
    created_at: v.number(),
    updated_at: v.number(),
  })
    // Índice para buscar conversación específica entre cliente y entrenador
    .index("by_client_trainer", ["client_user_id", "trainer_user_id"])
    // Índice para listar conversaciones del cliente ordenadas por último mensaje
    .index("by_client_last_message", ["client_user_id", "last_message_at"])
    // Índice para listar conversaciones del entrenador ordenadas por último mensaje
    .index("by_trainer_last_message", ["trainer_user_id", "last_message_at"])
  ...
  messages: defineTable({
    ...
    // Índice principal: mensajes de una conversación ordenados por fecha
    .index("by_conversation_created", ["conversation_id", "created_at"])
    // Índice para mensajes no leídos (optimización futura)
    .index("by_conversation_status", ["conversation_id", "status"]),

  // ==================== CHAT: CUOTAS DE MENSAJES ====================

  message_quotas: defineTable({
    conversation_id: v.id("conversations"),
    used_count: v.number(),
    reset_at: v.number(),
    created_at: v.number(),
