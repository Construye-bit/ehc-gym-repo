...
import {
    seedUser, seedRole, seedPerson, seedTrainer, seedPost, seedPostLike
...
describe("Posts: creación, consulta y likes", () => {
    ...
    it("createPost: solo trainers activos pueden crear publicaciones", async () => {
        ...
        const tUser = seedUser(db, { clerk_id: "trainer1", name: "Trainer", email: "t@x.com" });
        seedRole(db, { user_id: tUser, role: "TRAINER" });
        const tPerson = seedPerson(db, { user_id: tUser, name: "John", last_name: "Doe" });
        const trainer = seedTrainer(db, { person_id: tPerson, user_id: tUser, status: "ACTIVE" });
        ...
        const result = await runMutation(
            PostMutations.createPost as any,
            ctx,
            { description: "Mi primer post", image_storage_id: undefined }
        );
    ...
    it("createPost: falla si el usuario no es trainer", async () => {
        ...
        const cUser = seedUser(db, { clerk_id: "client1", name: "Client", email: "c@x.com" });
    ...
    it("getPost: devuelve data completa con user_has_liked", async () => {
        ...
        const post = seedPost(db, { trainer_id: trainer, user_id: tUser, description: "Test post", likes_count: 1 });

        // Usuario que dio like
        const likerUser = seedUser(db, { clerk_id: "liker1", name: "Liker", email: "l@x.com" });
        seedPostLike(db, { post_id: post, user_id: likerUser });

        // Consultar como el usuario que dio like
        db.setAuthUser("liker1");

        const result = await runQuery(PostQueries.getPost as any, ctx, { postId: post });
    ...
    it("getPost: user_has_liked es false si no dio like", async () => {
        ...
        const tPerson = seedPerson(db, { user_id: tUser, name: "John", last_name: "Doe" });
        const trainer = seedTrainer(db, { person_id: tPerson, user_id: tUser });
        const post = seedPost(db, { trainer_id: trainer, user_id: tUser, likes_count: 0 });

        // Otro usuario sin like
        const otherUser = seedUser(db, { clerk_id: "other1", name: "Other", email: "o@x.com" });
    ...
    it("toggleLike: like incrementa likesCount, unlike decrementa", async () => {
        ...
        // Primer toggle: LIKE
        const result1 = await runMutation(PostLikeMutations.toggleLike as any, ctx, { postId: post });
        ...
        const result2 = await runMutation(PostLikeMutations.toggleLike as any, ctx, { postId: post });
    ...
    it("toggleLike: no permite duplicados, mantiene consistencia", async () => {
        ...
        // Dar like
        await runMutation(PostLikeMutations.toggleLike as any, ctx, { postId: post });

        // Verificar que existe 1 solo like en la tabla
        const likes = db.table("post_likes").filter((l: any) => l.post_id === post && l.user_id === likerUser);
        ...
        const likesAfter = db.table("post_likes").filter((l: any) => l.post_id === post && l.user_id === likerUser);
