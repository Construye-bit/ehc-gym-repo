...
export function EditSedeModal({ isOpen, onOpenChange, branchId }: EditSedeModalProps) {
    ...
    const branchDetails = useQuery(api.branches.queries.getByIdForAdmin,
        branchId && isAuthenticated ? { branchId: branchId as Id<"branches"> } : "skip"
    );
    ...
    useEffect(() => {
        if (branchDetails && isInitializing) {
            ...
            setFormData({
                name: branch.name || "",
                cityId: (branch.address as any)?.city_id || "",
                addressId: branch.address_id || "",
                phone: branch.phone || "",
                email: branch.email || "",
                opening_time: branch.opening_time || "06:00",
                closing_time: branch.closing_time || "22:00",
                max_capacity: branch.max_capacity?.toString() || "100",
                status: branch.status || "ACTIVE",
                opening_date: branch.opening_date ? new Date(branch.opening_date).toISOString().split('T')[0] : "",
                metadata: {
                    has_parking: branch.metadata?.has_parking || false,
                    has_pool: branch.metadata?.has_pool || false,
                    has_sauna: branch.metadata?.has_sauna || false,
                    has_spa: branch.metadata?.has_spa || false,
                    has_locker_rooms: branch.metadata?.has_locker_rooms || true,
                    wifi_available: branch.metadata?.wifi_available || true,
                },
            });
    ...
    // Reset address when city changes (but keep current address if it belongs to the city)
    useEffect(() => {
        if (formData.cityId && !isInitializing) {
            // Only reset if the current address doesn't belong to the new city
            const currentAddress = addresses?.find(addr => addr._id === formData.addressId);
            if (!currentAddress) {
                setFormData(prev => ({ ...prev, addressId: "" }));
            }
        }
    }, [formData.cityId, addresses, isInitializing]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Sede actualizada exitosamente", {
                description: `La sede "${formData.name}" ha sido actualizada correctamente.`
            });
            ...
            <Dialog open={isOpen} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="flex justify-center items-center py-12">
                        ...
                        <span className="ml-2 text-white">Cargando datos de la sede...</span>
                    </div>
                </DialogContent>
            </Dialog>
        );
            ...
            <Dialog open={isOpen} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-md">
                    ...
                    <div className="text-center py-6">
                        <p className="text-white">No se pudo cargar la información de la sede.</p>
                    </div>
                    <DialogFooter>
